...
export default function TeamsPage() {
    ...
    try {
      ...
      if (savedTeams && savedTeams.columns) {
        ...
        zones = [
          ...
          { id: "zone-main-2", name: "โซน 2", type: "main", teamOrder: allMain.slice(6, 30) },
        ...
        zones = migrateToZones({ mainZone1Order: allMain.slice(0, 6), mainZone2Order: allMain.slice(6, 30), subOrder }, cols);
      } else {
  ...
  const handleSave = useCallback(async (currentData: DataState) => {
    ...
    try {
      ...
      const res = await axios.put("/api/castle", payload);
  ...
  const canAddMainTeam = mainPlayerCount < 150 && mainTeamCount < 30;
  ...
  return (
    <div className="space-y-6 bg-[#f0f6fc] dark:bg-[#1C1F27] min-h-screen p-4 lg:p-6 relative">
            ...
            <div className="flex-1">
                ...
                <div className="space-y-10 pb-12 bg-[#f0f6fc] dark:bg-[#1C1F27] print-export-padding">
                  ...
                  <div className="bg-white dark:bg-[#232733] rounded-xl border border-slate-200 dark:border-[#2D3342] p-3 flex flex-col gap-3 shadow-sm">
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-bold text-slate-700 dark:text-white whitespace-nowrap">สนามหลัก {mainPlayerCount}/150 คน</span>
                      <div className="flex-1 bg-slate-100 dark:bg-[#272C38] rounded-full h-2.5 overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${mainPlayerCount >= 150 ? "bg-emerald-500" : mainPlayerCount >= 120 ? "bg-amber-400" : "bg-[#3B66D1]"}`} style={{ width: `${Math.min(100, (mainPlayerCount / 150) * 100)}%` }} />
                      </div>
                      <span className={`text-xs font-bold ${mainPlayerCount >= 150 ? "text-emerald-500" : "text-slate-500"}`}>{mainPlayerCount >= 150 ? "เต็ม ✓" : `เหลือ ${150 - mainPlayerCount} ที่`}</span>
                    </div>
      ...
      {isClearConfirmOpen && (
        <div className="fixed inset-0 bg-black/150 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm animate-in fade-in duration-200">
